# Core_01
<< .Net Core 2.1  RESTful WEB API Service  >>

This application is a RESTful WEB API service using .Net Core 2.1.  The architecture consists of Controllers, Services, Repositories and a Database.
...
.Net Core, needs to set up the database connection in the appsetting.json and Startup.cs, which means, no need for Web.config.  In particular, the DbContext, the Repositories, the Interfaces are registered in the Startup.cs for implementing Dependency Injection.
